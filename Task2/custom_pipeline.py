...
class Generator4Embeds:
    ...
    def __init__(self, num_inference_steps=1, device='cuda') -> None:
        ...
        sdxl_local_path = "/Generation/models/sdxl-turbo"         
        # 确保这个路径下有 sdxl_models 文件夹（或者您存放权重的结构）
        ip_adapter_local_path = "/Generation/models/IP-Adapter"

        self.num_inference_steps = num_inference_steps
        self.dtype = torch.float16
        self.device = device

        pipe = DiffusionPipeline.from_pretrained(sdxl_local_path, torch_dtype=torch.float16, variant="fp16",local_files_only=True)
        ...
        try:
            ...
            print(f"[WARN] Failed to load IP-Adapter from {ip_adapter_local_path}: {e}")
            print("[WARN] Continuing without IP-Adapter. To enable IP-Adapter, download the full 'sdxl_models' folder from the HF repo 'h94/IP-Adapter'.")
